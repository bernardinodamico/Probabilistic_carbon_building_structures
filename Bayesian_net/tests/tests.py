...
class TestBuild_ProbTables(unittest.TestCase):
    ...
    fetch_pt = Fetch_ProbTables()
    pt.load_dataset(path="Bayesian_net/tests/dummy_dataset.csv")
    ...
    def test_marginal_probs(self):
        ...
        mpt = self.pt.bld_pr_table(vars=['Temp'])
        ...
        mpt2 = self.pt.bld_pr_table(vars=['Weather'])
        ...
        mpt3 = self.pt.bld_pr_table(vars=['Wildfire'])
        ...
        param = randint(0, 8)
        smoothed_mpt = self.pt.bld_pr_table(vars=['Weather'], K=param)
        smoothed_mpt, _ = self.get_probs_as_list(smoothed_mpt.table.to_dict())
        self.assertAlmostEqual(first=sum(smoothed_mpt), second=1., places=7, msg=f'Laplace smoothing error: marginal probs do not sum up to 1. with K={param}', delta=0.001)

        param = randint(0, 8)
        smoothed_mpt = self.pt.bld_pr_table(vars=['Wildfire'], K=param)
        smoothed_mpt, _ = self.get_probs_as_list(smoothed_mpt.table.to_dict())
        self.assertAlmostEqual(first=sum(smoothed_mpt), second=1., places=7, msg=f'Laplace smoothing error: marginal probs do not sum up to 1. with K={param}', delta=0.001)

    def test_joint_probs(self):
        jpt = self.pt.bld_pr_table(vars=['Temp', 'Weather', 'Wildfire'])
        ...
        jpt2 = self.pt.bld_pr_table(vars=['Temp', 'Weather'])
        ...
        jpt3 = self.pt.bld_pr_table(vars=['Temp', 'Wildfire'])
        ...
        jpt4 = self.pt.bld_pr_table(vars=['Weather', 'Wildfire'])
    ...
    def test_conditional_probs(self):
        ...
        cpt = self.pt.bld_cond_pr_table(var='Temp', given_vars=['Weather', 'Wildfire'])
        ...
        cpt2 = self.pt.bld_cond_pr_table(var='Temp', given_vars=['Wildfire'])
        ...
        param = randint(0, 8)
        smoothed_cpt2 = self.pt.bld_cond_pr_table(var='Temp', given_vars=['Weather'], K=param)
    ...
    def test_assign_evidence(self):
        ...
        cpt = self.pt.bld_cond_pr_table(var='Temp', given_vars=['Weather', 'Wildfire'])
        ass_cpt = self.ve.assign_evidence(prT=cpt, assignment_vals=ass_vars_vals)
        ...
        cpt2 = self.pt.bld_cond_pr_table(var='Temp', given_vars=['Weather', 'Wildfire'])
        ass_cpt2 = self.ve.assign_evidence(prT=cpt2, assignment_vals=ass_vars_vals2)
